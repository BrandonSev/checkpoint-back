...
const createToken = (id, business, data) => {
  return jwt.sign({ id, isBusiness: business, data }, process.env.TOKEN_SECRET, {
    expiresIn: maxAge,
  });
};
...
module.exports.signIn = async (req, res) => {
  ...
  try {
    ...
    if (business) {
      ...
      const token = createToken(business.id, true, business);
      if (comparison) {
    ...
    const token = createToken(user.id, false, user);
    if (comparison) {
      res.cookie("jwt", token, { httpOnly: true, secure: true, sameSite: true, maxAge });
      return res.status(200).json({
        message: "Connexion réussi",
